...
const transformAidTypeData = (distributions: Distribution[]): AidTypeDataPoint[] => {
...
const transformTimeSeriesData = (distributions: Distribution[]): TimeSeriesDataPoint[] => {
...
const transformAidTypeDataLegacy = (distributions: Distribution[]) => {
  const aidTypeCounts = distributions.reduce((acc, distribution) => {
    acc[distribution.aidType] = (acc[distribution.aidType] || 0) + 1;
    return acc;
  }, {} as Record<string, number>);
...
const transformTimeSeriesDataLegacy = (distributions: Distribution[]) => {
